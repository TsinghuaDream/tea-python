...
class TestStream(unittest.TestCase):
    ...
    def test_read_as_sse_success(self):
        stream = [
            {
                'response': mock.MagicMock(status_code=200, headers={'Content-Type': 'text/event-stream'}),
                'stream': b'data: message 1\n\n'
            },
            {
                'response': mock.MagicMock(status_code=200, headers={'Content-Type': 'text/event-stream'}),
                'stream': b'data: message 2\n\n'
            }
        ]
        
        results = list(Stream.read_as_sse(stream))
        self.assertEqual(len(results), 2)
        self.assertEqual(results[0]['event'].data, 'message 1')
        self.assertEqual(results[1]['event'].data, 'message 2')
        ...
        stream = [
            {
                ...
                'stream': b'{"Code": "400", "Message": "Bad Request", "RequestId": "12345"}\n'
            }
        ]

        with self.assertRaises(DaraException) as context:
            list(Stream.read_as_sse(stream))

        self.assertEqual(context.exception.message, "code: 400, Bad Request request id: 12345")

    async def test_read_as_sse_async_success(self):
        async def mock_stream():
            ...
            yield {
                'status_code': 200,
                'headers': {'Content-Type': 'text/event-stream'},
                'body': b'data: message 2\n\n'
            }

        results = []
        async for result in Stream.read_as_sse_async(mock_stream()):
